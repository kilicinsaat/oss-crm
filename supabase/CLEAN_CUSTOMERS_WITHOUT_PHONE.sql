...
-- Deletes customer cards that do not have a valid mobile number and also clears their history.
...
begin
  ...
  where customer_id in (
    ...
    where not (
      regexp_replace(coalesce(phone, ''), '\D', '', 'g') ~ '^5[0-9]{9}$'
      or regexp_replace(coalesce(phone_2, ''), '\D', '', 'g') ~ '^5[0-9]{9}$'
    )
  );
